...
const SHA256_core = struct {
    ...
    fn digest_block(self: *SHA256_core, m_block: [64]u8) void {
        ...
        for (16..self.w_block.len) |i| {
            const val = SHA256_core.sig_one(self.w_block[i-2]) +% self.w_block[i-7] +% SHA256_core.sig_zero(self.w_block[i-15]) +% self.w_block[i-16];
            self.w_block[i] = val;
        }
...
pub fn main() !void {
    ...
    _ = args.skip();
    const file_path = args.next() orelse return;
    ...
    var buf = [_]u8{0x80} ++ [_]u8{0} ** 63;
    var sha_core = SHA256_core{};

    while (true) {
        buf = [_]u8{0x80} ++ [_]u8{0} ** 63;
        const bytes_read = try in_stream.readAll(&buf);
        if (bytes_read == 0) break;

        sha_core.message_size += bytes_read * 8;

        if (bytes_read < (64 - 8 - 1)) {
            buf[bytes_read] = 0x80;

            for (std.mem.asBytes(&std.mem.nativeTo(u64, sha_core.message_size, .big)), 56..) |byte, i| {
                buf[i] = std.mem.nativeToBig(u8, byte);
            }

            sha_core.digest_block(buf);
        } 
        else if (bytes_read < 64) {
            buf[bytes_read] = 0x80;
            sha_core.digest_block(buf);
        } 
        else {
    ...
    std.debug.print("{s}\n", .{sha_core.getHashString(result_str_allocator)});
}
